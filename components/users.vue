  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Login Status', value: 'updated_at'},
      ],
      categories: [],
      editedIndex: -1,
      editedItem: {
    ...
    computed: {
      ...
      required_field : function (){
        if(this.editedItem.name != '' && this.editedItem.email != '' && this.editedItem.password != '' && this.editedItem.role != '' ){
          return true
        }
        return false;
      },
    ...
    methods: {
      ...
      async load_categories () {
          const send = await this.$axios.get('/backend/alluser');
      ...
      async save_data (){
        const send =  await this.$axios.post('/backend/register', {
